...
`;
  ...
  async function loadProgress(gameId) {
    ...
    try {
      const doc = await db.collection("gameProgress").doc(uid).collection("games").doc(gameId).get();
  ...
  async function saveProgress(gameId, p) {
    ...
    try { await db.collection("gameProgress").doc(uid).collection("games").doc(gameId).set(p, { merge: true }); }
...
.gsu-game .gg-geno{display:inline-flex;align-items:center;gap:9px;cursor:pointer;text-decoration:none;background:rgba(27,42,74,.7);border:1.5px solid rgba(201,168,76,.35);border-radius:999px;padding:8px 15px 8px 8px;color:var(--cream);font-size:14px;font-weight:600;transition:border-color .15s}
    ...
    _render(mount){
      ...
      mount.innerHTML=`
        ...
        <div class="gg-card" id="gg-card">
          ...
          <div class="gg-row">
            <a class="gg-geno" id="gg-geno" href="#" aria-label="Ask GENO about this"><img src="${GENO_PNG}" alt="GENO"><span>Ask GENO</span></a>
      ...
      mount.querySelector("#gg-geno").addEventListener("click",(e)=>{e.preventDefault();const cur=s.current;document.dispatchEvent(new CustomEvent("geno:open",{detail:{context:`${c.title} — ${s.tiers[s.progress.tier].name}. Question: ${cur?cur.prompt:""}`}}));});
